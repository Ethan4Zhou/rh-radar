...
python3 docs/build_site.py         # one self-contained page
if [[ -n "$(git status --porcelain data docs)" ]]; then
  ...
  if ! git push -q origin main 2>/dev/null; then
    ...
    if git rebase -q origin/main 2>/dev/null; then
      ...
      git checkout --ours  data/accounts.json data/contract_verify.json docs/data.json docs/index.html 2>/dev/null || true
      git add -A data docs 2>/dev/null || true
